...
export function RestaurantCard({
  ...
  /**
   * Handle click on closed restaurant - show toast notification
   */
  const handleClosedRestaurantClick = (e: React.MouseEvent) => {
    e.preventDefault();
    toastNotification.info(
      t("restaurant.closedWarning", "Restaurant is currently closed"),
      t("restaurant.closedWarningDetail", "This restaurant is not accepting orders at the moment. Please try again later when they are open.")
    );
  };

  // If restaurant is closed, render with click handler instead of Link
  if (!isOpen) {
    return (
      <div className="block h-full cursor-pointer" onClick={handleClosedRestaurantClick}>
        <Card
          className={cn(
        ...
        >
          ...
          <div className="relative w-full h-48 bg-muted">
            ...
            {/* Closed badge */}
            <div className="absolute top-2 right-2 z-10 bg-green-500 text-white text-xs font-semibold px-2 py-1 rounded">
  ...
  // If restaurant is open, render with Link for navigation
  return (
